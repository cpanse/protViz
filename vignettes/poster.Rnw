...
<<label=insilico, eval = TRUE, fig = TRUE, include=FALSE, width=9, height=9, echo=FALSE>>=
par <- par(mfrow = c(3, 1))
...
{\centering
\includegraphics[width=0.8\columnwidth, keepaspectratio]{graphics/protViz-insilico}}
...
<<label=featureDensityPlot, keep.source = TRUE, eval = TRUE, fig = TRUE, width=8, height=5>>=
par(mfrow=c(1,1)); 
data(pgLFQfeature)
data(pgLFQprot)
...
<<keep.source = TRUE, eval = TRUE, fig = TRUE, include=TRUE, width=6, height=4>>=
data(iTRAQ)
par(mfrow = c(2,4),
    mar = c(6,4,3,0.5));
for (i in 3:10){
...
<<label=iTRAQboxplot, keep.source = TRUE, eval = TRUE, fig = TRUE, include=FALSE, width=8, height=4>>=
...
\centering
\resizebox{1.0\columnwidth}{!}{
	\includegraphics{graphics/protViz-iTRAQboxplot}
}
}
...
<<keep.source = TRUE, eval = TRUE, fig = TRUE, width=8, height=3, include=TRUE, echo=FALSE>>=
data(iTRAQ)
