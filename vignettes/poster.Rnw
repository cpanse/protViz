%\VignetteIndexEntry{poster (in portrait)}
\pdfminorversion=4
...
%\minipage{
   \begin{center}
     \textsf{
     \textbf{\huge{\Rcode{protViz}: Visualizing and Analyzing Mass Spectrometry Related Data in Proteomics in R}}\\[1.0cm]
...
\setkeys{Gin}{width=\columnwidth, height=0.5\columnwidth}
...
\resizebox{1.0\columnwidth}{!}{
	\includegraphics{graphics/protViz-digest}
}
...
<<>>=
defaultIon
peptides <- c('HTLNQIDSVK', 'ALGGEDVR',
'TPIVGQPSIPGGPVR')
pim <- parentIonMass(peptides)
fi <- fragmentIon(peptides)
@

<<label=insilico, keep.source = TRUE, eval = TRUE, fig = TRUE, include=FALSE, width=9, height=9, echo=FALSE>>=
par(mfrow=c(3,1)); 
...
\resizebox{1.0\columnwidth}{!}{
	\includegraphics{graphics/protViz-insilico}
}
...
@
\resizebox{1.0\columnwidth}{!}{
	\includegraphics{graphics/protViz-peakplot}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The data set \Rcode{fetuinLFQ} contains a subset of our results descriped in \cite{20576481}.
...
@
\resizebox{1.0\columnwidth}{!}{
	\includegraphics{graphics/protViz-LFQtrellis}
}
...
This figure shows the correlation between runs on feature level (values are asinh transformed). White is perfect correlation while black indicates a poor correlation.
...
This figure shows the correlation between runs on protein level (values are \Rcode{asinh} transformed). White is perfect correlation while black indicates a poor correlation.
Striking is the fact that the six biological replicates for each condition cluster very well.

{\tiny
\setkeys{Gin}{width=0.60\columnwidth}
<<keep.source = TRUE, eval = TRUE, fig = TRUE>>=
