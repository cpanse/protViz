...
lcmsmap <- function(data, charges=2:3, score.cutoff = 30, ...){
  ...
  s.charge <- NULL
    ...
    if ('mascot' %in% class(data)){
      ...
      s.score <-  (unlist(lapply(data$queries, function(x){rv <- x$q_peptide$pep_score; if(is.null(rv)){NA}else{as.numeric(rv)}})))
      s.charge <- as.integer(gsub("[+]", "", unlist(lapply(data$queries, function(x){(x$query_charge)}))))
    }else{
      if (is.null(sub)){sub <- data[[1]]$title}
  ...
  S <- data.frame(rt = s.rtinseconds,
    ...
    for (c in charges){
        ...
        legend("topleft", c(paste('score.cutoff = ', score.cutoff), 'rest'), lwd=c(4), col=c(rgb(0.1,0.1,0.8, alpha=0.9), 'grey')) 
    }
}
